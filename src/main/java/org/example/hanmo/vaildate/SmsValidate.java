...
public class SmsValidate {
    ...
    public static void validateSmsCodeExistence(String certificationCode, RedisSmsRepository redisSmsRepository) {
        ...
        if (value == null || value.trim().isEmpty()) {
            throw new SmsSendException("400_Error, 인증번호가 만료되었습니다.", ErrorCode.SMS_VERIFICATION_FAILED_EXCEPTION);
        }
    }
    ...
    //인증과 , 임시토큰 함께 검증
    public static String validateSmsCodeByCode(String certificationCode, RedisSmsRepository redisSmsRepository) {
        String phoneNumber = redisSmsRepository.getSmsCertification(certificationCode);
        if (phoneNumber == null) {
            throw new SmsSendException("400_Error, 인증번호가 만료되었거나 일치하지 않습니다.", ErrorCode.SMS_VERIFICATION_FAILED_EXCEPTION);
        }
        return phoneNumber;
    }
}
